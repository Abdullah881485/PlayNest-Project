...
const ToyCard = ({ card }) => {
  return (
    <div
      key={card.toyId}
      className="flex flex-col pt-3 border border-gray-300 rounded-lg shadow-md hover:shadow-2xl transition-all gap-3 bg-white cursor-pointer duration-300"
    >
      <img
        src={card.pictureURL}
        alt={card.toyName}
        className="rounded-xl w-full h-30 md:h-60 object-contain"
      />
      ...
      <div className="card-actions w-full flex justify-center">
        <Link
          ...
          className="py-2 shine-btn bg-[#ff6f61] hover:bg-[#4D96FF] rounded-b-lg font-bold text-white w-full text-center"
        >
          View More
        </Link>
      </div>
